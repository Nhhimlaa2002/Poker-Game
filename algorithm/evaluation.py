...
def evaluate_situation(state):
    if state.get("phase") == "folded":
        pot = state.get("pot", 0)
        # If the opponent (min) folded, that's a WIN for our AI (max) -> positive.
        # If our AI (max) folded, that's a LOSS -> negative.
        return pot if state.get("folded_by") == "min" else -pot
